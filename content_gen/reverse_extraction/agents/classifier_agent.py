...
class ClassifierAgent:
    ...
    def _fallback_classification(
        ...
        try:
            ...
            # Если нашли блок по ключевым словам, но LLM не определил - используем найденный
            if found_block and not result.thematic_block:
                result.thematic_block = found_block
                logger.info(f"Использован блок {found_block}, найденный по ключевым словам")

            # Если LLM определил блок, но он не совпадает с найденным - логируем предупреждение
            if found_block and result.thematic_block and found_block != result.thematic_block:
                logger.warning(
                    f"Несоответствие блоков: найден по ключевым словам={found_block}, "
                    f"определен LLM={result.thematic_block}. Используем LLM результат."
                )

            logger.info(
                f"ClassifierAgent: language={result.language}, "
                f"thematic_block={result.thematic_block or result.thematic_block_suggested}, "
                f"audience_level={result.audience_level}, project_type={result.project_type}"
            )
            ...
            logger.error(f"Ошибка при классификации: {e}")
            # Fallback: используем найденный блок или базовые значения
            return ClassificationResult(
