...
5. Используем только следующие допустимые типы Mermaid:
...
6. Формулы в LaTeX (только содержимое, без $$)
7. Таблицы в Markdown формате (полный блок)
8. НЕ используй старый синтаксис "graph" без типа направления
9. Все должно быть релевантно теме
10. НЕ дублируй формулы/таблицы, которые уже есть в других частях (см. выше)
11. Весь ответ — строго один JSON-объект без текста до/после него.
...
"""
    ...
    def _normalize_mermaid(self, mermaid_code: str) -> str:
        ...
        # ШАГ 6: Добавляем минимальный размер шрифта для читаемости (если еще не добавлен)
        # Минимальный размер шрифта: 16px для текста внутри блоков
        if "%%{init}" not in code and "%%{" not in code:
            ...
            if any(diagram_type in first_line for diagram_type in ['flowchart', 'graph', 'sequenceDiagram', 'stateDiagram']):
                # Добавляем темную тему с белыми элементами
