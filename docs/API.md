...
#### POST /reverse-extract/extract

Извлекает структурированные данные из README.md и создает Excel файл со спецификацией проекта.
...
curl -X POST "http://localhost:8000/api/v1/reverse-extract/extract" \
  ...
  -d '{"readme_text": "# Проект\n\n## Глава 1\n..."}'
```

#### GET /reverse-extract/download/{file_id}

Скачивает сгенерированный Excel файл со спецификацией проекта.

**Требует аутентификации:** Да

**Параметры:**
- `file_id` (path) — ID файла из ответа `/reverse-extract/extract`

**Ответ:**
- Content-Type: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`
- Content-Disposition: `attachment; filename=project_spec.xlsx`
- Excel файл с заполненной спецификацией

**Пример:**
```bash
curl -X GET "http://localhost:8000/api/v1/reverse-extract/download/{file_id}" \
  -H "Authorization: Bearer <token>" \
  -o project_spec.xlsx
```

**Примечания:**
- Файлы хранятся в in-memory кэше
- `file_id` действителен до перезапуска сервера
- Метаданные включают предупреждения и ошибки валидации
