...
def test_gateway_passes_openrouter_credentials_base_url_headers_and_temperature(monkeypatch) -> None:
    monkeypatch.delenv("OPEN_ROUTER_TEMPERATURE", raising=False)
    ...
    monkeypatch.setenv("OPEN_ROUTER_SITE_URL", "https://content-generator.example")
    monkeypatch.setenv("OPEN_ROUTER_APP_NAME", "Content Generator Test")
    monkeypatch.setenv("OPENROUTER_TEMPERATURE", "0.2")
    gateway = LLMGateway(enable_cache=False)
    route = ModelRoute(provider="openrouter", model="openai/gpt-5.4-mini")

    request_kwargs = gateway._litellm_request_kwargs(route, {})

    assert request_kwargs["api_key"] == "openrouter-key"
    assert request_kwargs["api_base"] == "https://openrouter.example/api/v1"
    assert request_kwargs["temperature"] == 0.2
    assert request_kwargs["extra_headers"] == {
        "HTTP-Referer": "https://content-generator.example",
        "X-Title": "Content Generator Test",
    }
    assert route.litellm_name() == "openrouter/openai/gpt-5.4-mini"
